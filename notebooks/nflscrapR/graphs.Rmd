...
```{r}
Barkley_Taylor_Fantasy <- fantasy %>% 
  filter(full_name %in% c("Saquon Barkley", "Jonathan Taylor"),
         gameday <= as.Date("2021-01-03") | gameday >= as.Date("2021-09-01")) %>% 
  ggplot(aes(x = gameday, y = fantasy_points_halfppr, fill = full_name)) +
    geom_col(position = "dodge", width = 5)

ggplotly(Barkley_Taylor_Fantasy)
...
p.mat <- cor_pmat(model.matrix(~0+., data=dynasty_college))

model.matrix(~0+., data=dynasty_college) %>%
  cor(use="pairwise.complete.obs") %>%
  ggcorrplot(show.diag = F, type="upper", p.mat = p.mat, insig = "blank")
```
